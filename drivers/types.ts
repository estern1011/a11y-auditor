...
export const STATE_KEYWORDS: readonly string[] = [
  "not selected", "has popup",
  "checked", "unchecked", "expanded", "collapsed",
  "selected", "dimmed", "required", "visited",
];
...
export function parseVoResponse(spoken: string, itemText: string): VoResponse {
  ...
  if (m) { role = m[1]; parsedName = text.slice(0, m.index); }
...
export function parseOrcaResponse(
  spoken: string, name: string, role: string, atspiState: string[],
): VoResponse {
  return {
