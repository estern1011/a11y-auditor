...
import {
  ...
  STARTUP_POLL_MS, STARTUP_POLL_MAX, SPEECH_RATE_KEY,
...
// ---------------------------------------------------------------------------
// HTTP helpers — throw on error, let cli() handle exit
// ---------------------------------------------------------------------------
  ...
  try {
    switch (cmd) {
      ...
      case "kill": {
        ...
        // Read persisted state for speech rate restoration and VO ownership
        let weStartedVO = true;
        let originalSpeechRate: string | null = null;
        try {
          ...
          if (typeof stateData.originalSpeechRate === "string") {
            originalSpeechRate = stateData.originalSpeechRate;
          }
        } catch {}
        // Restore speech rate before quitting VoiceOver
        if (originalSpeechRate !== null) {
          spawnSync("defaults", ["write", "com.apple.VoiceOver4/default", SPEECH_RATE_KEY, "-int", originalSpeechRate]);
        }
        if (weStartedVO) {
