...
- **Secrets flow.** *Partially clarified (slice #4).* The agent nodes talk to Claude via the Claude Agent SDK, which spawns Claude Code as a subprocess. Claude Code authenticates via any of: a cached OAuth / subscription login under `~/.claude/`, `CLAUDE_CODE_OAUTH_TOKEN`, `ANTHROPIC_API_KEY`, Vertex, or Bedrock — the runner does no pre-flight check because the SDK's own auth error is richer than anything env inspection could produce. (a) Local Mac runs reuse the user's existing Claude Code login under `~/.claude/`; no extra configuration. (b) Sprite runs need the Claude Code credentials present in the sprite — simplest is to have `eval/sprite-bootstrap.sh` rsync `~/.claude/` into the sprite before running the runner; alternatively export `CLAUDE_CODE_OAUTH_TOKEN` through `sprite exec` env. (c) The server hosts the dispatcher only and never handles the Claude key itself. Still open: how to expose "logged into Claude?" health to the UI when dispatch uses a non-default auth path.
- **Driver port allocation.** *Resolved in slice #3.* The runner picks two free ports via `net.createServer().listen(0)` + immediate close (`runner/tools/driver.ts` → `findFreePortPair`), spawns `bun drivers/{sr}/driver.ts serve <url> --port <p> --cdp-port <c>` directly (not through the `start` launcher), polls the daemon's HTTP healthcheck, and returns a `stop()` handle wired into `runner/index.ts`'s finally. Writes the real ports into `meta.json` on boot for the screencast endpoint to read.
- **Driver pid/state file collision on parallel runs.** *Deferred.* `drivers/{orca,voiceover}/core.ts` hardcode `/tmp/{orca,voiceover}-driver.pid` and `/tmp/{orca,voiceover}-driver-state.json`. Two concurrent runs on one host — which Phase A's web dashboard will eventually trigger from a single local server — clobber each other regardless of whether the HTTP port is free. Fix options: (a) suffix the paths with the daemon's port, (b) accept a `--state-dir` flag, (c) relocate both into the caller-provided `runDir`. Any of the three touches the auditor skill's direct-invocation flow (`bun {sr-driver} stop`/`kill` read the well-known paths) and needs a doc update. Blocks parallel local runs; not a blocker for Phase A if we serialize runs in the server.
- **Viewport plumbing.** *Deferred.* `runner/state.ts` carries `viewport: { w, h }` and `runner/tools/driver.ts` accepts it in `StartDriverInput`, but nothing applies it — `collect.ts` never calls `page.setViewportSize`, and the driver's `/navigate` endpoint takes only a URL. Needs either a driver-level viewport flag (passed at `serve` time so every tab inherits it) or a new `/viewport` endpoint the runner hits after boot. Belongs with the slice that first depends on non-default viewports (keyboard / responsive audits).
- **Screenshot fallback in `collect.ts`.** *Deferred.* `collect.ts:writeEvidenceToDir` silently routes non-`data:image/png;base64,…` output into `artifacts/screenshot.txt` instead of failing. On the sprite smoke run the saved file was 96 bytes of plain text, not an image — the dashboard's Evidence tab will render nothing. Fix: fail loudly on unexpected content (or normalize via agent-browser's PNG path) so the symptom surfaces during collect, not at the UI.
...
- **Cost envelope.** *Partially in place (slice #4).* Each run fans out to ~3 Agent SDK sessions plus the `auth` session when triggered. `runner/agent-host.ts` now emits a `budget` event line per agent with `total_cost_usd` + total tokens from the SDK's `result` message, and caps each agent at a 5-minute wall-clock timeout via `AbortController`. No hard per-run token cap yet — the abort above a configurable ceiling is the next step, along with surfacing the cumulative budget in the UI.
...
- **Agent output → state contract.** Conditional edges depend on signals the agent must return (`needsAuth`, `hasInteractive`, `treeEmpty`). Mitigation: each agent node validates its Agent SDK output against a Zod schema before returning the state patch; on validation failure the node fails loudly. *Slice #4:* baseline-collector has its `runner/schemas/baseline-collector.ts` contract (Zod + JSON-Schema twin handed to the SDK's `outputFormat`). `runner/agent-host.ts` throws with the offending payload in the message so bad runs are inspectable without re-running. Keyboard + visual schemas land with the next slice.
