...
- **Cost envelope.** Each run fans out to ~3 Agent SDK sessions plus the `auth` session when triggered. No per-run token cap or cost estimate. *Partial in slice #4:* `runner/agent-host.ts` accumulates each session's token + cost usage and emits one `budget` event per agent (`{ node, agentId, usd, tokens }`). Still missing: per-run rollup in state and the configurable ceiling that aborts the graph when the budget is blown. Per-session timeout is in (5 minutes via `Query.interrupt()`); a hard budget abort belongs with the LangGraph `AbortSignal` work under Cancel/timeout.
- **Structured output enforcement.** *Slice #4.* agent-host currently validates the final `result.result` string against a Zod schema. The Agent SDK supports a native `outputFormat: { type: 'json_schema', schema }` option that would enforce the shape before it reaches us. Deferred: generate the JSON schema from the Zod schema (`zod-to-json-schema`) and pass it through `Options.outputFormat` so misformatted agent output fails in the SDK, not in our `safeParse`.
- **Raw-message redaction in `agent-<id>.ndjson`.** *Slice #4.* runner/agent-host.ts writes every SDK message verbatim to `runs/<id>/agent-<id>.ndjson` for post-mortem inspection. Good for debugging; bad once the auth node starts typing credentials into tool-use arguments. Phase B's auth design note needs to cover redaction at the write boundary (drop `tool_use.input` for `Bash`/keystroke tools when the agent is `auth`, or scrub header/cookie values by regex before append).
