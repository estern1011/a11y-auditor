 ...
 *
 * Communicates with AT-SPI2's accessibility bus using the
 * org.a11y.atspi.Accessible D-Bus interface. Resolves the bus address via
 * the session bus → org.a11y.Bus.GetAddress() chain.
 */
...
async function getConnection(): Promise<any> {
  ...
  a11yBus = dbus.createClient({ busAddress: address });
  return a11yBus;
}
...
/**
 * Tell Orca to enable sticky browse mode via its D-Bus service. Browse mode
 * enables structural navigation (h=heading, k=link) and element-by-element
 * reading with arrow keys.
 *
 * Must be called AFTER focus is inside a web document, otherwise Orca
 * responds with "Not in a document."
 */
...
/**
 * Send a keyboard event through AT-SPI2's DeviceEventController. Unlike
 * xdotool (which uses XTEST), this goes through the AT-SPI2 D-Bus pipeline,
 * so Orca actually sees and responds to the keystroke.
 */
export async function generateKeyboardEvent(key: string, modifiers: string[] = []): Promise<void> {
  ...
  // For single printable characters (e.g. 'h' for heading quick-nav), use
  // the character's Unicode code point as the keysym.
...
function getProperty(
  ...
  return callMethod(bus, dest, path, "org.freedesktop.DBus.Properties", "Get", "ss", [iface, prop]);
}
...
async function getChildAtIndex(
  bus: any,
  dest: string,
  path: string,
  index: number,
